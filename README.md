...
| Reciever | Unavailable Solvers | A request made by this reciever to solve a token couldn't be executed, as all solvers were occupied at the time the solve was requested. Wait until one becomes available (you get the result from one back). Structure: <0>. |
